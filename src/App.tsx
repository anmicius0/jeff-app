...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink flex flex-col font-sans selection:bg-primary/30">
      ...
      {/* Main Content Area */}
      <main className="flex-1 max-w-4xl w-full mx-auto p-3 sm:p-6 space-y-3 sm:space-y-4 pb-24">
        {/* Navigation Tabs (Linear Pricing/Pill tab token) */}
        <div className="flex justify-center">
          <div className="inline-flex p-1 rounded-pill bg-surface-1 border border-hairline">
            <button
              onClick={() => setActiveTab('workout')}
              className={`px-3.5 sm:px-5 py-1 sm:py-1.5 rounded-pill text-xs font-medium transition duration-150 flex items-center gap-1.5 ${
                activeTab === 'workout'
        ...
        {activeTab === 'workout' ? (
          <>
            ...
            <div className="rounded-lg bg-surface-1 border border-hairline p-3 space-y-2 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-1 text-xs">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                  <span className="font-semibold text-ink truncate max-w-[120px] sm:max-w-none">{currentDay.name}</span>
                  <span className="text-[10px] font-mono text-ink-subtle px-1.5 py-0.2 rounded bg-surface-2 border border-hairline whitespace-nowrap">
                    C{currentCycle} · W{currentWeek}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 font-mono text-[11px] sm:text-xs">
                  <span className="text-primary-hover font-bold">{sessionProgressPct}%</span>
            ...
            <div className="rounded-lg bg-surface-1 border border-hairline p-3 space-y-3 shadow-lg">
              ...
              <div className="flex items-center justify-between gap-1.5 sm:gap-2">
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-1.5 rounded bg-surface-2 hover:bg-surface-3 disabled:opacity-30 border border-hairline text-ink transition"
                    title="Previous Week"
                  >
                  ...
                  <button
                    ...
                    className="text-xs font-mono font-semibold px-1.5 sm:px-2 py-1 rounded bg-surface-2 hover:bg-surface-3 border border-hairline text-primary-hover whitespace-nowrap transition cursor-pointer"
                ...
                <div className="flex gap-1 overflow-x-auto scrollbar-none py-0.5 min-w-0 flex-1 px-1">
                  {WORKOUT_PLAN_DATA.map((w) => (
                    <button
                      key={w.weekNumber}
                      type="button"
                      onClick={() => handleSelectWeek(w.weekNumber, currentCycle)}
                      className={`px-2 py-1 rounded text-[11px] font-mono border transition shrink-0 ${
                        currentWeek === w.weekNumber
                          ? 'bg-primary text-on-primary border-primary font-bold'
                          : 'bg-surface-2 hover:bg-surface-3 border-hairline text-ink-subtle'
                      }`}
                    >
                      W{w.weekNumber}
                    </button>
              ...
              {showManualDateInput && (
                <div className="flex flex-wrap items-center justify-between gap-2 p-2 rounded bg-surface-2 border border-hairline text-xs font-mono animate-in fade-in">
                  <span className="text-ink-subtle flex items-center gap-1.5">
                    <Calendar className="w-3 h-3 text-primary-hover" /> Session Date:
                  </span>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="bg-surface-3 border border-hairline-strong text-ink rounded px-2 py-1 text-xs outline-none focus:border-primary"
                    />
                    <button
                      type="button"
                      onClick={() => setSelectedDate(getTodayLocalDateString())}
                      className="px-2 py-1 rounded bg-surface-3 hover:bg-surface-4 text-ink-subtle text-[11px]"
                    >
                      Today
                    </button>
                  </div>
              ...
              <div className="flex gap-1.5 overflow-x-auto scrollbar-none pt-1 border-t border-hairline">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs text-ink-subtle px-1">
                <span>Exercises ({currentDay.name})</span>
                ...
                {currentDay.exercises.map((rawEx, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 px-3 py-2 rounded-md border text-xs text-left transition flex items-center gap-2 ${
                        isSelected
                          ? 'bg-surface-2 border-primary/50 text-ink ring-1 ring-primary/30'
                          : 'bg-surface-1 hover:bg-surface-2 border-hairline text-ink-muted'
                      }`}
                    >
                      <span className="w-4 h-4 rounded-full bg-surface-3 flex items-center justify-center text-[10px] font-mono">
                        {isDone ? (
                          <Check className="w-3 h-3 text-semantic-success stroke-[3]" />
                        ) : (
                          idx + 1
                        )}
                      </span>
                      <span className="truncate max-w-[130px] font-medium">{exName}</span>
                    </button>
                  );
                })}
              </div>
